...
class Auth extends CI_Controller 
{
	...
	public function registration()
	{
		...
		$this->session->set_flashdata(
			...
			congratulation, your account has been created. Please acctivate your account!
			</div>'
			);
			redirect('auth');
		}
	...
	// funtion untuk mengirim verifikasi ke Email
	private function _sendEmail($token, $type)
	{
		$config = [
            'protocol'  => 'smtp',
            'smtp_host' => 'ssl://smtp.googlemail.com',
            'smtp_user' => 'tibalagi7@gmail.com',
            'smtp_pass' => 'vqywwhzzwfnvlind',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
            'newline'   => "\r\n"
		...
		// $config['smtp_user'] = 'tibalagi7@gmail.com'; //email yang dirubah keamanannya
		// $config['smtp_pass'] = 'vqywwhzzwfnvlind'; //isikan password dr kelola akun gmail
		...
		$this->email->from('tibalagi7@gmail', 'Userku NGirim');
		...
		if ($type == 'verify') {
			...
			$this->email->message('Clik this link to verify you account :
				...
				'&token=' . urlencode($token) . '">Aktivated</a>');
		} else if ($type == 'forgot') {
			$this->email->subject('Reset Password');
